...
//                count = 1;
    ...
    private static readonly TweenLerp<float>.Lerp FloatLerp = InternalFloatLerp;
    private static readonly TweenLerp<int>.Lerp IntLerp = InternalIntLerp;
    private static readonly TweenLerp<Vector3>.Lerp Vector3Lerp = InternalVector3Lerp;
    private static readonly TweenLerp<Vector2>.Lerp Vector2Lerp = InternalVector2Lerp;
    private static readonly TweenLerp<Color>.Lerp ColorLerp = InternalColorLerp;

    private static float InternalFloatLerp(float start, float end, float t)
    {
        return start + (end - start) * t;
    }

    private static int InternalIntLerp(int start, int end, float t)
    {
        return start + (int)((float)(end - start) * t);
    }

    private static Vector3 InternalVector3Lerp(Vector3 start, Vector3 end, float t)
    {
        return start + (end - start) * t;
    }

    private static Vector2 InternalVector2Lerp(Vector2 start, Vector2 end, float t)
    {
        return start + (end - start) * t;
    }

    private static Color InternalColorLerp(Color start, Color end, float t)
    {
        return start + (end - start) * t;
    }
...
public struct TweenLerp<T>
{
    ...
    public T Value
    {
        get { return this.tweener.SwapStartEnd ? this.lerp(this.end, this.start, this.tweener.LastLerp) : this.lerp(this.start, this.end, this.tweener.LastLerp); }
    }

    public SimpleTweener Tweener => this.tweener;
}